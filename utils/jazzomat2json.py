...
from pychord_tools import labels
...
def beatTimestampsFromJSON(jsonFile):
    with open(jsonFile, 'r') as f:
        ...
        allBeats = []
        allChords = []
        common_utils.processParts(metreNumerator, data, allBeats, allChords, 'chords')
...
def degree(root, bass):
    rootN = labels.note_to_number(root)
    bassN = labels.note_to_number(bass)
    degree = bassN - rootN
    if (degree < 0):
        degree += 12
